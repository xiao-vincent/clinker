...
public class PaymentValidator implements ConstraintValidator<ValidPayment, Payment> {
	...
	private boolean isPaymentOverRemainingBalance(Payment payment, ConstraintValidatorContext context) {
		...
		if (paymentAmt.compareTo(invoiceBalance) > 0) {
			String msg = "Payment amount of " + paymentAmt + " is greater than remaining invoice balance of " + invoiceBalance;
